...
template<typename T>
class Board : public IBoard<T>{
    std::vector<std::vector<std::vector<T>>> m_board{};
...
public:
    ...
    [[nodiscard]] std::vector<std::vector<std::vector<T>>>& getBoard() {
        return m_board;
    }
    ...
    [[nodiscard]] bool isAPile(const size_t _row, const size_t _col) const {
        return m_board[_row][_col].size() >= 2;
    }
    [[nodiscard]] bool checkIndexes(const size_t _row, const size_t _col) const {
        return !(_row >= this->m_board.size() || _col >= this->m_board.size());
    }
    [[nodiscard]] bool checkNeighbours(const size_t _row, const size_t _col) const {
    ...
    [[nodiscard]] bool checkValue(const size_t _row, const size_t _col, const typename T::Value _value, const bool _illusion) const {
        if (m_board[_row][_col].empty() || m_board[_row][_col].back().getValue() == T::Value::Border)
            return true;

        if (_illusion)
            return false;

        if (m_board[_row][_col].back().getValue() == T::Value::Eter || _value == T::Value::Eter)
            return false;
    ...
    [[nodiscard]] bool checkIllusion(const size_t _row, const size_t _col, const T::Color _color) const {
        if (m_board[_row][_col].empty())
            return false;

        auto& topCard = m_board[_row][_col].back();

        if (topCard.getColor() == T::Color::Undefined)
            return false;

        return topCard.isIllusion() && topCard.getColor() == _color;
    }

    [[nodiscard]] bool checkIllusionValue(const size_t _row, const size_t _col, const size_t _value) const {
        if (m_board[_row][_col].empty() || m_board[_row][_col].back().getColor() == T::Color::Undefined)
            return true;

        return static_cast<size_t>(m_board[_row][_col].back().getValue()) < _value;
    }

    [[nodiscard]] typename T::Color checkWin() const {
        ...
        for (int sum : sums) {
            if (sum == n) return T::Color::Player1;
            if (sum == -n) return T::Color::Player2;
        }
    ...
    [[nodiscard]] typename T::Color calculateWinner() const {
        ...
        for (std::size_t row = 0; row < this->m_board.size(); ++row) {
            for (std::size_t col = 0; col < this->m_board.size(); ++col) {
                if (!m_board[row][col].empty()) {
                    ...
                    if (cardValue == static_cast<size_t>(T::Value::Eter))
                        cardValue = 1;
                    ...
                    if (topCard.getColor() == T::Color::Player1) {
        ...
        if (winner.first > winner.second)
            return T::Color::Player1;

        if (winner.first < winner.second)
            return T::Color::Player2;
    ...
    void useExplosion(std::vector<T>& returnedCards, std::vector<T>& eliminatedCards) {
    ...
    std::unordered_map<Explosion::ExplosionEffect, std::function<void(size_t, size_t)>> effectHandlers{
        ...
        {Explosion::ExplosionEffect::RemoveCard,[this, &eliminatedCards](size_t row, size_t col) {
            T affectedCard = this->m_board[row][col].back();
    ...
    [[nodiscard]] bool checkPartial(const size_t _x, const size_t _y, const size_t _int_value) const {
        if (!this->checkIndexes(_x, _y) || this->checkHole(_x, _y))
            return false;

        if (_int_value > static_cast<size_t>(typename T::Value::Four))
            return false;

        const auto value = static_cast<T::Value>(_int_value);
    ...
    std::optional<T> placeCard(const size_t _row, const size_t _col, const T&& _card) {
        m_board[_row][_col].push_back(_card);
        std::optional<T> card = std::nullopt;
