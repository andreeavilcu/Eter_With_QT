...
Player::Player(const Card::Color _color, const std::vector<Card>& _cards, const bool _wizard, const bool _powers) :
    ...
    m_wizard_index = _wizard ? /*wizardDistribution(gen)*/ 0 : -1;/*1 - 3*/
    m_powers_index.first = _powers ? /*powerDistribution(gen) : -1*/ 3 : -1; /*0 - 7*/

    do {
        m_powers_index.second = _powers ? /*powerDistribution(gen)*/ 1 : -1;
